...
static inline void HC_PMU_config_counter(const uint8_t counter, const uint32_t r_budget, const uint32_t w_budget, const uint8_t event, const uint8_t task_num)
{
	...
	ASM (
		...
		"hvc #0\n\t"
		:
		: "r" (hc_id), "r" (counter), "r" (r_budget), "r" (w_budget), "r" (event), "r" (task_num)
		: "x0", "x1", "x2", "x3", "x4", "x5"
		);
}
...
static inline uint32_t HC_regulator_periodic_timer_handler(const uint32_t th1, const uint32_t th2, const uint32_t counter)
{
	const uint32_t hc_id = SMCC64_FID_VND_HYP_SRVC | HC_REGULATOR_PERIODIC_TIMER_HANDLER;
	...
	ASM (
		...
		"hvc #0\n\t"
		:
		: "r" (hc_id), "r" (th1), "r" (th2), "r" (counter)
		: "x0", "x1", "x2", "x3"
		);	
}


static inline void HC_regulator_check_counter(const uint32_t task_num, const uint32_t counter, const uint8_t op_type)
{
	...
	ASM (
		...
		"hvc #0\n\t"
		:
		: "r" (hc_id), "r" (task_num), "r" (counter), "r" (op_type)
		: "x0", "x1", "x2", "x3"
		);
}


static inline uint32_t HC_regulator_period_expired(const uint32_t task_num, const uint8_t op_type )
{
...
static inline uint32_t HC_regulator_get_new_budget(const uint8_t task_num, const uint8_t op_type)
{
...
static inline uint32_t HC_regulator_get_total_used_budget(const uint8_t task_num, const uint8_t op_type)
{
	const uint32_t hc_id = SMCC64_FID_VND_HYP_SRVC | HC_REGULATOR_GET_TOTAL_USED_BUDGET;
	uint32_t ret = 0;
...
static inline uint32_t HC_regulator_get_current_used_budget(const uint8_t task_num, const uint8_t op_type)
{
	const uint32_t hc_id = SMCC64_FID_VND_HYP_SRVC | HC_REGULATOR_GET_CURRENT_USED_BUDGET;
	uint32_t ret = 0;
...
static inline uint64_t HC_regulator_get_total_calculated_new_budget(const uint8_t task_num, const uint8_t op_type)
{
	const uint32_t hc_id = SMCC64_FID_VND_HYP_SRVC | HC_REGULATOR_GET_TOTAL_CALCULATED_NEW_BUDGET;
	uint32_t ret = 0;
