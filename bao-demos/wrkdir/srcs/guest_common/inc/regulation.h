...
struct VM {
  const uint32_t r_fac[TASK_QNT];
  const uint32_t w_fac[TASK_QNT];
  uint8_t sgi_suspend_task_budget[TASK_QNT];
  const uint32_t total_read_allowed_budget;
  const uint32_t total_write_allowed_budget;
  uint32_t new_read_budget[TASK_QNT];
  uint32_t new_write_budget[TASK_QNT];
  uint32_t deadline_met_counter[TASK_QNT];
  uint32_t deadline_missed_counter[TASK_QNT];
  uint32_t cycle_per_period[TASK_QNT][PERIOD_QNT];
  uint32_t task_start_time[TASK_QNT][PERIOD_QNT];
  uint32_t task_end_time[TASK_QNT][PERIOD_QNT];
  uint32_t task_deadline[TASK_QNT][PERIOD_QNT];
  uint32_t used_r_budget_period[TASK_QNT][PERIOD_QNT];
  uint32_t used_w_budget_period[TASK_QNT][PERIOD_QNT];
  uint32_t calc_r_budget_period[TASK_QNT][PERIOD_QNT];
  uint32_t calc_w_budget_period[TASK_QNT][PERIOD_QNT];
};
