...
void fs_set_type(int type) { fs_type = type; }
...
static inline int fs_ls_unsupported(const char *dirname) { return -1; }

/* generic implementation of ls in terms of opendir/readdir/closedir */
__maybe_unused static int fs_ls_generic(const char *dirname) {
...
static inline int fs_uuid_unsupported(char *uuid_str) { return -1; }
...
static inline int fs_unlink_unsupported(const char *filename) { return -1; }

static inline int fs_mkdir_unsupported(const char *dirname) { return -1; }
 ...
 */
int fs_get_type(void) { return fs_type; }
 ...
 */
const char *fs_get_type_name(void) { return fs_get_info(fs_type)->name; }
...
int do_load(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[],
  ...
  if (IS_ENABLED(CONFIG_CMD_BOOTEFI))
    efi_set_bootdev(argv[1], (argc > 2) ? argv[2] : "",
                    (argc > 4) ? argv[4] : "", map_sysmem(addr, 0), len_read);

  printf("inside u-boot, argc == %d\n", argc);
