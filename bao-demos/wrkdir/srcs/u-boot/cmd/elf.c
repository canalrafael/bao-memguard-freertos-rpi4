...
int do_bootelf(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[]) {
  ...
  /* Check for flag. */
  if (argc >= 1 &&
      (argv[0][0] == '-' && (argv[0][1] == 'p' || argv[0][1] == 's'))) {
    sload = argv[0];
    /* Consume flag. */
    argc--;
    argv++;
  ...
  printf("Inside do_bootelf, argc %d\n", argc);
...
int do_bootvx(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[]) {
  ...
  info->addr =
      (info->entries - 1) * sizeof(struct e820_entry) + E820_DATA_OFFSET;
  ...
  if (!bootline) {
    ...
    if (tmp) {
      ...
      if (tmp) {
        ...
        ptr += sprintf(build_buf + ptr, ":%08x ", ntohl(mask));
      } else {
...
U_BOOT_CMD(bootelf, CONFIG_SYS_MAXARGS, 0, do_bootelf,
           "Boot from an ELF image in memory",
           "[-p|-s] [address]\n"
           "\t- load ELF image at [address] via program headers (-p)\n"
           "\t  or via section headers (-s)");

U_BOOT_CMD(bootvx, 2, 0, do_bootvx, "Boot vxWorks from an ELF image",
