...
#endif
#define debug_bootkeys(fmt, args...) debug_cond(DEBUG_BOOTKEYS, fmt, ##args)
  ...
  do {
    if (tstc()) {
      ...
      if ((presskey[presskey_len] == '\r') ||
        ...
        if (IS_ENABLED(CONFIG_AUTOBOOT_NEVER_TIMEOUT) && !presskey_len) {
          never_timeout = 1;
          continue;
        }
        presskey[presskey_len] = '\0';
        err = crypt_compare(crypt_env_str, presskey, &abort);
        if (err)
          debug_bootkeys("crypt_compare() failed with: %s\n", errno_str(err));
        /* you had one chance */
        break;
      } else {
        presskey_len++;
...
static int passwd_abort_sha256(uint64_t etime) {
  ...
  do {
    if (tstc()) {
      ...
      hash_block(algo_name, (const void *)presskey, presskey_len, sha, &size);
...
static int passwd_abort_key(uint64_t etime) {
  ...
  for (i = 0; i < sizeof(delaykey) / sizeof(delaykey[0]); i++) {
    ...
    presskey_max =
        presskey_max > delaykey[i].len ? presskey_max : delaykey[i].len;
  ...
  do {
    ...
    for (i = 0; i < sizeof(delaykey) / sizeof(delaykey[0]); i++) {
      if (delaykey[i].len > 0 && presskey_len >= delaykey[i].len &&
          memcmp(presskey + presskey_len - delaykey[i].len, delaykey[i].str,
                 delaykey[i].len) == 0) {
