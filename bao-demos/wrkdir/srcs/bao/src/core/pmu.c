...
void PMU_interrupt_handler()
{
	...
	if (overflow_reg_val & (1 << 0) || overflow_reg_val & (1 << 1))
		vcpu_inject_irq(cpu()->vcpu, GUEST_SUSPEND_TASK_0_BUDGET_ID);
	else
...
void PMU_config_counter(const uint8_t counter, uint32_t r_budget, uint32_t w_budget, const uint8_t event, const uint8_t task_num)
{
	PMU_config();

	reg_conf[cpu()->id].vm[task_num].defined_pmu_read_val = r_budget;
	reg_conf[cpu()->id].vm[task_num].defined_pmu_write_val = w_budget;
